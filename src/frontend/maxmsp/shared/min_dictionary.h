...
namespace c74::min {
    ...
    public:
        ...
        min_dict(max::t_dictionary* d = nullptr, const bool take_ownership = true) {
            if (d == nullptr)
        ...
        min_dict(const atom an_atom_containing_a_dict) {
        ...
        min_dict& operator=(const min_dict& value) {
            max::dictionary_clone_to_existing(value.m_instance, m_instance);
            return *this;
        }


        min_dict& operator=(const atom& value) {
        ...
        void copyunique(const min_dict& source) {
